  ...
  album: function(data, cssPath) {
    ...
    while (++j < rowNum) {
      ...
      while (++i < colNum) {
        ...
        console.log(`/beverage_images/${data[++k]}.png`);
        var imgPath = `<img src = /beverage_images/${data[k]}.png class="cover">`;
/*
...
*/
        ...
        ret += `<div class="item">`;
        ret += `<div>1</div>`;
        ret += `<div><button type="button" id=${k}>삭제</button></div>`;
        ret += `</div>`; 
        ret += `</div>`;
        ret += `
        <script>
          document.getElementById(${k}).addEventListener("click",function() {
            axios.post('/album_delete', {idx: ${k}, status: 2}).then((result) => {
